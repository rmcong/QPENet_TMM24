...
def validate(val_loader, model, val_seed):
    ...
    for e in range(db_epoch):
        for i, (input, target, s_input, s_mask, padding_mask, s_padding_mask, cat_idx_list, ori_label) in enumerate(val_loader):
            ...
            if isinstance(input, list):    # multi-scale test
                ...
                for scale_id in range(scale_len):
                    ...
                    with torch.no_grad():
                        output_temp = model(s_x=s_input_temp, s_y=s_mask_temp, x=input_temp, y=target_temp, classes=cat_idx_list, padding_mask=padding_mask, s_padding_mask=s_padding_mask, prototype_neg_dict=None)
                ...
                start_time = time.time()
                output = model(s_x=s_input, s_y=s_mask, x=input, y=target, padding_mask=padding_mask, s_padding_mask=s_padding_mask, classes=cat_idx_list, prototype_neg_dict=None)
                model_time.update(time.time() - start_time)
