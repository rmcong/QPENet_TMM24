...
# ----------------------  TRAINVAL  ----------------------
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        loss_train, mIoU_train, mAcc_train, allAcc_train, prototype_neg_dict = train(train_loader, val_loader, model, optimizer, epoch, train_data.sub_list, prototype_neg_dict=prototype_neg_dict)
        ...
        if (epoch % args.save_freq == 0) and (epoch > 0) and main_process():
            if args.shot == 1:
                ...
                filename = args.snapshot_path + '/epoch_5_{}.pth'.format(epoch)
        ...
        # save model for <testing> and <fine-tuning>
            if mIoU > best_miou:
                ...
                if args.shot == 1:
                    ...
                    filename = args.snapshot_path + '/train_{}_epoch_'.format(args.shot) + str(epoch) + '_{:.4f}'.format(best_miou) + '.pth'
...
def train(train_loader, val_loader, model, optimizer, epoch, base_list,prototype_neg_dict=None):
    ...
    for i, (input, target, s_input, s_mask, padding_mask, s_padding_mask, subcls) in enumerate(train_loader):
        ...
        output, main_loss, aux_loss, prototype_neg_dict = model(s_x=s_input, s_y=s_mask, x=input, y=target, classes=subcls, padding_mask=padding_mask, s_padding_mask=s_padding_mask, prototype_neg_dict=prototype_neg_dict)
...
def validate(val_loader, model):
    ...
    for e in range(10): 
        for i, (input, target, s_input, s_mask, padding_mask, s_padding_mask, subcls, ori_label) in enumerate(val_loader):
            ...
            with torch.no_grad():
                output = model(s_x=s_input, s_y=s_mask, x=input, y=target, classes=subcls, padding_mask=padding_mask, s_padding_mask=s_padding_mask, prototype_neg_dict=None)
